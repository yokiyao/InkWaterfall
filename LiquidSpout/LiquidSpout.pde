...
Spout spout;
  ...
  public void setup(){
    ...
    for(int i = 0; i < mobs.length; i++){
      float r = 50 + i*15;
      mobs[i] = new MouseObstacle(i, random(r, width-r), random(r,height-r)* 2, r);
    }
    ...
    particles.param.velocity_damping  = 0.95f;
...
//    }
  ...
  public void autoSpawnParticles(){
    ...
    //    sr.num(1);
    //  sr.dim(30, 30);
    //  sr.pos(width-px-110, height-1 - py- 105);
    //  sr.vel(0, -6);
    //  particles.spawn(width, height, sr);
...
//    spawn(0,1);
  ...
  public void keyReleased(){
    ...
    if(key == 'l') APPLY_LIQUID_FX = true;
    if(key == ';') APPLY_LIQUID_FX = false;
  }
